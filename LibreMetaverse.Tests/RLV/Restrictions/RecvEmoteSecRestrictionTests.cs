...
namespace LibreMetaverse.Tests.RLV.Restrictions
{
    ...
    public class RecvEmoteSecRestrictionTests : RlvTestBase
    {
        ...
        public async Task CanRecvChat_RecvEmote_Secure()
        {
            ...
            await _rlv.ProcessMessageAsync("@recvemote_sec=n", _sender.Id, _sender.Name);
            await _rlv.ProcessMessageAsync($"@recvemote:{userId1}=add", sender2.Id, sender2.Name);
            await _rlv.ProcessMessageAsync($"@recvemote:{userId2}=add", _sender.Id, _sender.Name);
