using System;
using System.Threading.Tasks;
using NUnit.Framework;
namespace LibreMetaverse.Tests.RLV.Exceptions
{
    [TestFixture]
    public class ShowNameTagsExceptionTests : RlvTestBase
    {
        #region @shownametags:uuid=<rem/add>
        [Test]
        public async Task CanShowNameTags_Except()
        {
            var userId1 = new Guid("00000000-0000-4000-8000-000000000000");
            var userId2 = new Guid("11111111-1111-4111-8111-111111111111");

            await _rlv.ProcessMessageAsync("@shownametags=n", _sender.Id, _sender.Name);
            await _rlv.ProcessMessageAsync($"@shownametags:{userId1}=add", _sender.Id, _sender.Name);

            Assert.That(_rlv.Permissions.CanShowNameTags(null), Is.False);
            Assert.That(_rlv.Permissions.CanShowNameTags(userId1), Is.True);
            Assert.That(_rlv.Permissions.CanShowNameTags(userId2), Is.False);
        }
        #endregion
    }
}
